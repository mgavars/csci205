...
public class Euler {
	...
	double slowEulerAnswer = 0.0;

	for (int i = 1; i<= numTerms; i++) {
		slowEulerAnswer += computeEulerTerm(i, x);
	}
        return 1.0 + slowEulerAnswer;
    }
    ...
    public static double computeEulerFast(double x, int numTerms) {
        // TODO - Complete this method!
	double fastEulerAnswer = 0.0, numerator = 1.0, denominator = 1.0;

	for (int i = 1; i<= numTerms; i++) {
		numerator *= x;
		denominator *= i;
		fastEulerAnswer += numerator / denominator;
	}
	fastEulerAnswer += 1.0;
        return fastEulerAnswer;
    }
	...
	// Define BigDecimal variables
	BigDecimal numerator = new BigDecimal(1.0);
	BigDecimal denominator = new BigDecimal(1.0);
	BigDecimal term = new BigDecimal(1.0);
	BigDecimal bigEulerAnswer = new BigDecimal(1.0);

	for (int i = 1; i <= numTerms; i++) {

		// Find new numerator and denominator	
		numerator = numerator.multiply(new BigDecimal(x));
		denominator = denominator.multiply(new BigDecimal(i));

		// Get the new fraction and add it to the final answer
		term = numerator.divide(denominator,20, RoundingMode.valueOf("HALF_UP"));
		bigEulerAnswer = bigEulerAnswer.add(term);
	}
        return bigEulerAnswer;
    }
